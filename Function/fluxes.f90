   ...
   do m=nx_start,nx_end 
     if(lcu(m,n)>0.5) then
      surf_stress_x(m,n)=(taux(m,n)+taux(m+1,n))/RefDen/2.0d0     &
                       *( (uwnd(m  ,n)*dx(m  ,n)*dy(m  ,n)         &
                          +uwnd(m+1,n)*dx(m+1,n)*dy(m+1,n))/2.0d0/dxt(m,n)/dyh(m,n) -uup(m,n,1) )     
     endif
     if(lcv(m,n)>0.5) then
      surf_stress_y(m,n)=(tauy(m,n)+tauy(m,n+1))/RefDen/2.0d0     &
                       *( (vwnd(m,n  )*dx(m,n  )*dy(m,n  )         &
                          +vwnd(m,n+1)*dx(m,n+1)*dy(m,n+1))/2.0d0/dxh(m,n)/dyt(m,n) -vvp(m,n,1) )     
     endif
   enddo
 enddo
 !$omp end parallel do
  ...
  !$omp parallel do private(m, n) reduction(+:sf_ave)
   do n=ny_start,ny_end
     do m=nx_start,nx_end 
       if(lu(m,n)>0.5) then
        sflux_surf(m,n)=-wf_tot(m,n)*ssp(m,n,1)/RefDen                &
